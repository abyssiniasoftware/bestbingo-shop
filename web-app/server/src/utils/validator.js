...
const houseValidation = [
  ...
  check("houseAdminId")
    .isMongoId()
    .withMessage("Invalid house admin ID")
    .custom(async (id) => {
...
//   check('houseId').isMongoId()
//   .withMessage('Invalid house ID')
//   .custom(async (id) => {
//     const house = await require('../models/House').findById(id);
//     if (!house ) {
...
//   check('cashierId')
//     .isMongoId()
//     .withMessage('Invalid house cashier ID')
//     .custom(async (id) => {
//       const user = await require('../models/User').findById(id);
...
const rechargeValidation = [
  check("houseId").isMongoId().withMessage("Invalid house ID"),
  check("amount")
    .isFloat({ min: 0.01 })
    .withMessage("Amount must be greater than 0"),
];
const rechargeUpdateValidation = [
  check("rechargeId").optional().isMongoId().withMessage("Invalid recharge ID"),
  check("amount")
    .isFloat({ min: 0.01 })
    .withMessage("Amount must be greater than 0"),
...
const isIdValid = (...keys) => {
  return keys.map((key) =>
    param(key).isMongoId().withMessage(`${key} must be a valid Mongo ID`)
  );
};
module.exports = {
