...
const HouseList = () => {
  ...
  useEffect(() => {
    if (rechargeModalOpen) {
      const randomThreeDigit = Math.floor(100 + Math.random() * 900); // Generates a 3-digit number
  ...
  const handleRechargeSubmit = async () => {
    // Validate verification code (simple match)
    if (!isVerified) {
      setRechargeMessage("Verification incorrect. Please try again.");
      setRechargeError(true);
      return;
        ...
        >
          ...
          <DialogContent sx={{ p: { xs: 1, sm: 2 } }}>
            ...
            >
              Recharge for:{" "}
              <strong>
                {houses.find((h) => h._id === selectedHouseId)?.name ||
                  "Unknown House"}
              </strong>
            </Typography>
            {verificationMessage && (
