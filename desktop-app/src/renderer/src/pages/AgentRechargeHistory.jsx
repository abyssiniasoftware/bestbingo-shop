...
const AgentRechargeHistory = () => {
  ...
  const handleRechargeSubmit = async () => {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        setMessage(
          `Recharge ${modalMode === "create" ? "submitted" : "updated"
          } successfully!`,
        );
        setError(false);
        // Refresh recharges
      ...
      setMessage(
        `An error occurred during recharge ${modalMode === "create" ? "" : "update"
        }.`,
      );
      setError(true);
    } finally {
  ...
  const stats = useMemo(() => {
    ...
    const houseTotals = (houses || []) // Guard added here
      .map((house) => ({
        name: house.name,
  ...
  const barChartData = useMemo(() => {
    const houseAmounts = (houses || []) // Guard added here
      .map((house) => ({
        name: house.name,
        amount: filteredRecharges
          .filter((r) => r.houseId?._id === house._id)
...
export default AgentRechargeHistory;
